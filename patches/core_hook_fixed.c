		     ...
		     unsigned long arg4, unsigned long arg5)
{
...
#ifdef CONFIG_KSU_SUSFS
	int susfs_cmd_err = 0;
	void __user *susfs_user_ptr = (void __user *)arg3;
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (arg2 == CMD_SUSFS_ADD_SUS_PATH) {
		susfs_add_sus_path((void __user **)&susfs_user_ptr);
		if (copy_to_user((void __user*)arg5, &susfs_cmd_err, sizeof(susfs_cmd_err)))
			pr_info("susfs: copy_to_user() failed\n");
		return 0;
	}
	if (arg2 == CMD_SUSFS_ADD_SUS_PATH_LOOP) {
		susfs_add_sus_path_loop((void __user **)&susfs_user_ptr);
		if (copy_to_user((void __user*)arg5, &susfs_cmd_err, sizeof(susfs_cmd_err)))
			pr_info("susfs: copy_to_user() failed\n");
		return 0;
	}
	...
	if (arg2 == CMD_SUSFS_HIDE_SUS_MNTS_FOR_NON_SU_PROCS) {
		susfs_set_hide_sus_mnts_for_non_su_procs((void __user **)&susfs_user_ptr);
		if (copy_to_user((void __user*)arg5, &susfs_cmd_err, sizeof(susfs_cmd_err)))
			pr_info("susfs: copy_to_user() failed\n");
		return 0;
	}
	...
	if (arg2 == CMD_SUSFS_ADD_SUS_KSTAT) {
		susfs_add_sus_kstat((void __user **)&susfs_user_ptr);
		if (copy_to_user((void __user*)arg5, &susfs_cmd_err, sizeof(susfs_cmd_err)))
			pr_info("susfs: copy_to_user() failed\n");
		return 0;
	}
	if (arg2 == CMD_SUSFS_UPDATE_SUS_KSTAT) {
		susfs_update_sus_kstat((void __user **)&susfs_user_ptr);
		if (copy_to_user((void __user*)arg5, &susfs_cmd_err, sizeof(susfs_cmd_err)))
			pr_info("susfs: copy_to_user() failed\n");
		return 0;
	}
	...
	if (arg2 == CMD_SUSFS_SET_UNAME) {
		susfs_set_uname((void __user **)&susfs_user_ptr);
		if (copy_to_user((void __user*)arg5, &susfs_cmd_err, sizeof(susfs_cmd_err)))
			pr_info("susfs: copy_to_user() failed\n");
		return 0;
	}
	...
	if (arg2 == CMD_SUSFS_ENABLE_LOG) {
		susfs_enable_log((void __user **)&susfs_user_ptr);
		if (copy_to_user((void __user*)arg5, &susfs_cmd_err, sizeof(susfs_cmd_err)))
			pr_info("susfs: copy_to_user() failed\n");
		return 0;
	}
	...
	if (arg2 == CMD_SUSFS_SET_CMDLINE_OR_BOOTCONFIG) {
		susfs_set_cmdline_or_bootconfig((void __user **)&susfs_user_ptr);
		if (copy_to_user((void __user*)arg5, &susfs_cmd_err, sizeof(susfs_cmd_err)))
			pr_info("susfs: copy_to_user() failed\n");
		return 0;
	}
	...
	if (arg2 == CMD_SUSFS_ADD_OPEN_REDIRECT) {
		susfs_add_open_redirect((void __user **)&susfs_user_ptr);
		if (copy_to_user((void __user*)arg5, &susfs_cmd_err, sizeof(susfs_cmd_err)))
			pr_info("susfs: copy_to_user() failed\n");
		return 0;
	}
	...
	if (arg2 == CMD_SUSFS_SHOW_VERSION) {
		susfs_show_version((void __user **)&susfs_user_ptr);
		if (copy_to_user((void __user*)arg5, &susfs_cmd_err, sizeof(susfs_cmd_err)))
			pr_info("susfs: copy_to_user() failed\n");
		return 0;
	}
	if (arg2 == CMD_SUSFS_SHOW_ENABLED_FEATURES) {
		susfs_get_enabled_features((void __user **)&susfs_user_ptr);
		if (copy_to_user((void __user*)arg5, &susfs_cmd_err, sizeof(susfs_cmd_err)))
			pr_info("susfs: copy_to_user() failed\n");
		return 0;
	}
	if (arg2 == CMD_SUSFS_SHOW_VARIANT) {
		susfs_show_variant((void __user **)&susfs_user_ptr);
		if (copy_to_user((void __user*)arg5, &susfs_cmd_err, sizeof(susfs_cmd_err)))
			pr_info("susfs: copy_to_user() failed\n");
		return 0;
	}
	...
	if (arg2 == CMD_SUSFS_ADD_SUS_MAP) {
		susfs_add_sus_map((void __user **)&susfs_user_ptr);
		if (copy_to_user((void __user*)arg5, &susfs_cmd_err, sizeof(susfs_cmd_err)))
				pr_info("susfs: copy_to_user() failed\n");
		return 0;
	}
	...
	if (arg2 == CMD_SUSFS_ENABLE_AVC_LOG_SPOOFING) {
		susfs_set_avc_log_spoofing((void __user **)&susfs_user_ptr);
		if (copy_to_user((void __user*)arg5, &susfs_cmd_err, sizeof(susfs_cmd_err)))
			pr_info("susfs: copy_to_user() failed\n");
		return 0;
	}
...
LSM_HANDLER_TYPE ksu_handle_setuid(struct cred *new, const struct cred *old)
{
	...
	if (ksu_is_manager_uid_valid() &&
		(new_uid.val % 1000000 == ksu_get_manager_uid()))
	{
		return 0;
	}
